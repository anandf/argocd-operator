...
import (
	...
	"github.com/argoproj-labs/argocd-operator/controllers/util"
...
func (r *ReconcileGitopsService) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if !util.IsConsoleAPIFound() {
		logs.Info("Console API not found skipping reconciliation for console plugin and gitops-backend")
		return reconcile.Result{}, nil
	}

	// Reconcile console plugin and gitops-backed only for OCP/OKD environments where ConsoleAPI CRD is available.
	if result, err := r.reconcileBackend(gitopsserviceNamespacedName, instance, reqLogger); err != nil {
		return result, err
	}

	isSupported, err := util.IsDynamicPluginSupported(r.Client)
	if isSupported {
		return r.reconcilePlugin(instance, request)
	}
	if err != nil {
		logs.Error(err, "error evaluating if dynamic plugin is supported, skipping reconciliation of console plugin related resources")
	}
	return reconcile.Result{}, nil
}
...
func (r *ReconcileGitopsService) ensureDefaultArgoCDInstanceDoesntExist(instance *pipelinesv1alpha1.GitopsService, reqLogger logr.Logger) error {
	...
	defaultArgoCDInstance, err := NewCR(common.ArgoCDInstanceName, serviceNamespace)
	if err != nil {
		return err
	}
